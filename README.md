...
Research Workbench 将文献阅读、知识管理、模型对话和受控文件操作整合到本地桌面工作台中。应用面向个人研究与开发任务，支持持续会话、来源检索、论文可视化与写入审批，保留研究过程中的原始记录和操作轨迹。

当前 0.4.13 客户端、Python 包及安装文件使用 `AgentWorkbench` 名称，现有安装标识和数据目录保持不变。

**版本：0.4.13** · Windows x64 · Python + FastAPI + WebView2 · 本地 E5 / ONNX · Three.js

> 应用以 Windows 桌面安装包交付，界面运行在原生 pywebview/WebView2 窗口中。安装版内置应用运行时，无需另行安装 Python 或 Node.js。

![聊天与工作区](agent_workbench/docs/screenshots/02-chat-workspace.png)
...
1. **受控执行**：通过结构化工具、写入审批、取消机制和冲突检查，管理模型对工作区的操作。
2. **知识积累**：保存原始会话，以独立摘要管理上下文预算，通过 PDF 与知识卡片检索定位来源。
3. **本地桌面工作流**：提供原生窗口、托盘、单实例运行和论文时间树，使用本地存储与离线前端资源。

动态引导是可选的响应偏好功能，根据语义锚点匹配注入提示词，不用于判定任务的客观难度。论文树按时间与主题组织文献，不表示引用关系或学术谱系。
...
![欢迎曲面](agent_workbench/docs/screenshots/01-welcome.png)

文件预览与写入审批是不同环节。模型调用 `edit_file` 后，先展示预期变更，再决定批准或拒绝，不通过“提示模型小心”替代执行约束。

| 文件预览 | 写入审批 |
| --- | --- |
| ![文件预览](agent_workbench/docs/screenshots/03-file-preview.png) | ![写入审批](agent_workbench/docs/screenshots/05-write-approval.png) |
...
| 上下文详情 | 文件与图片附件 |
| --- | --- |
| ![上下文详情](agent_workbench/docs/screenshots/04-context.png) | ![附件](agent_workbench/docs/screenshots/16-attachments.png) |

### 3. 知识库与卡片

卡片保存可复用结论；模型通过 `draft_card` 提出候选及保存理由，用户查看、编辑后确认。知识库还提供索引一致性审计和受控修复。

![知识库](agent_workbench/docs/screenshots/08-knowledge-library.png)

| 编辑卡片 | 审阅模型草稿 |
| --- | --- |
| ![卡片编辑](agent_workbench/docs/screenshots/06-knowledge-card.png) | ![卡片草稿](agent_workbench/docs/screenshots/07-card-draft.png) |

### 4. 论文时间树与原文阅读

每篇论文保留标题、年份、出处和主题。三维视图适合浏览，列表适合扫描；详情页可以人工修订元数据、请求模型分类、查看字段依据，或限定到该论文发起检索问答。

![论文时间树](agent_workbench/docs/screenshots/09-paper-tree.png)

![论文列表](agent_workbench/docs/screenshots/10-paper-list.png)

| 论文详情 | 本地 PDF 阅读 |
| --- | --- |
| ![论文详情](agent_workbench/docs/screenshots/11-paper-detail.png) | ![PDF 阅读](agent_workbench/docs/screenshots/12-paper-reader.png) |
...
| 模型与应用设置 | 三档示例及引导 |
| --- | --- |
| ![设置](agent_workbench/docs/screenshots/13-settings.png) | ![动态引导设置](agent_workbench/docs/screenshots/14-routing-settings.png) |

![路由解释](agent_workbench/docs/screenshots/15-route-details.png)

输入区模型列表是独立弹层，不必跳回设置；可取消而不强制选择。首次关闭窗口可选择退出或隐藏到托盘，错误信息使用原生对话框并按需展开详情。

![输入区模型选择](agent_workbench/docs/screenshots/17-model-picker.png)

| 原生关闭选择 | 原生错误详情（测试 fixture） |
| --- | --- |
| ![关闭选择](agent_workbench/docs/screenshots/18-native-close.png) | ![错误详情](agent_workbench/docs/screenshots/19-native-error.png) |
...
具体依赖以 [requirements.txt](agent_workbench/requirements.txt)、[package-lock.json](agent_workbench/package-lock.json) 和离线资源清单为准。Node.js、Playwright 是开发/验证工具，不是安装版运行依赖。项目没有使用 React、Electron、LangChain、LangGraph 或外部向量数据库。
...
| 文件/目录 | 职责 |
| --- | --- |
| [app.py](agent_workbench/app.py)、[bootstrap.py](agent_workbench/bootstrap.py) | 原生窗口、本机服务启动/退出、依赖组装 |
| [runtime.py](agent_workbench/runtime.py)、[server/](agent_workbench/server/) | 会话级状态、互斥操作、API 与 SSE |
| [core/loop.py](agent_workbench/core/loop.py) | 模型流解析、工具调用、取消与执行循环 |
| [core/workspace.py](agent_workbench/core/workspace.py) | 路径边界、文件读写与审批 |
| [core/history.py](agent_workbench/core/history.py)、[core/compaction.py](agent_workbench/core/compaction.py) | 原始历史、摘要 checkpoint 与上下文构造 |
| [router/](agent_workbench/router/) | 引用过滤、窗口、显式规则、语义排名与弃权 |
| [knowledge/](agent_workbench/knowledge/) | PDF、卡片、索引一致性和论文元数据 |
| [providers/](agent_workbench/providers/) | 模型接口与推理档位映射 |
| [assets/web/](agent_workbench/assets/web/) | 原生窗口中的 UI、三维论文树和交互状态 |
| [tests/](agent_workbench/tests/)、[scripts/](agent_workbench/scripts/) | 分层回归、安装版检查与可复现截图 |
...
完整实验报告见 [动态引导：阈值校准与保留验证](agent_workbench/docs/ROUTING-VALIDATION.md)，包含预先确定的协议、4,539 组阈值搜索、逐档结果、误触发分析与公开逐例证据。
...
保留集按档位的正确触发分别为：简洁 **1/4**、标准 **0/4**、深入 **4/4**。两条负向误触发分别为“这个我还没决定。”（简洁）和“把表头里的空格替换成下划线。”（深入）。分差从 0.013 降至 0.012 时，校准集会新增 2 条误触发，说明当前参数存在邻域敏感性。

**验证范围。** 当前参数提高了校准集的正向覆盖，但未达到最初设定的“保留集零误触发”标准。保留集仍有 2 条负向用例误触发，标准档覆盖有限；本版本采用该参数作为覆盖率与保守性之间的阶段性取舍。该验证仅衡量固定内部用例的路由结果，不代表泛化能力或引导对回答质量的提升。原冻结标签和结果保持不变。
...
本次版本对应的测试和产物记录见 [发布记录](agent_workbench/docs/RELEASE-0.4.13.md)。文档截图可用 `scripts/capture_docs.py` 配合 `scripts/capture_docs.cjs` 重建，需要 Playwright；它们只创建独立的合成演示数据。

## 典型工作流

1. **建立研究工作区**：选择本地目录，在对话中读取、搜索和引用项目文件。
2. **整理文献**：导入 PDF，检查提取状态，按需生成分类并修订标题、年份、出处和主题。
3. **阅读与检索**：通过论文时间树或列表定位文献，在阅读器中查看原文，并限定文献范围进行检索问答。
4. **沉淀知识**：将可复用结论整理为知识卡片；模型提出的草稿经用户审阅后保存并索引。
5. **执行修改**：查看文件变更并批准或拒绝写入；继续会话时保留原始历史，通过摘要管理模型上下文。

## 已知限制与后续工作

- 当前面向单用户本地使用，不包含团队权限、多租户、云同步、远端执行集群或任意 Shell 工具。
...
- 公开源码不包含依赖个人环境路径的历史实验脚本；固定路由用例与冻结配置位于 `validation/restrained-v1/`。
...
- 本项目原创代码采用 [MIT License](agent_workbench/LICENSE)，版权署名为 AgentWorkbench contributors。该许可不覆盖第三方组件、模型权重或微软运行时的各自条款，不能将整个捆绑安装包简单视为仅受 MIT 约束。
- 依赖许可见 [THIRD_PARTY_NOTICES.md](agent_workbench/THIRD_PARTY_NOTICES.md)。特别注意 PyMuPDF 的 AGPL/商业授权选项及模型、微软运行时的各自条款；本 README 不替代许可审查。
